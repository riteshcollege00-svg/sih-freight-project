...
weekly seasonality/noise, and random volatility spikes.
"""
...
def generate_synthetic_rates(
    ...
    -----------
    commodity : str
        The bulk commodity type (e.g., 'coal', 'iron_ore').
    origin : str
        Origin port or country (e.g., 'Australia').
    destination_port : str
        Destination port (e.g., 'Paradip').
    start_date : str
    ...
    """
    # Combine inputs into a deterministic seed offset to allow route-specific variation
    route_hash = abs(hash((commodity, origin, destination_port))) % 10000
    np.random.seed(seed + route_hash)

    # Base parameters for freight rate (USD / tonne)
    base_rate = 15.0 + (route_hash % 10)  # Typical freight rate around 15-25 USD/tonne
